    ...
    private function processCategoryTree(array $productsData, array $apiCategories)
    {
        ...
        foreach ($tree as $groupName => $subGroups) {
            $id_group = (int)Db::getInstance()->getValue('
                ...
                WHERE cl.name = "'.pSQL($groupName).'" AND c.id_parent = '.(int)$this->victronParentCategoryId
            );

            if (!$id_group) {
                $category = new Category();
                $category->name = array_fill_keys(Language::getIDs(true), $groupName);
                $category->link_rewrite = array_fill_keys(Language::getIDs(true), Tools::str2url($groupName));
            ...
            foreach ($subGroups as $subGroupName) {
                $id_subgroup = (int)Db::getInstance()->getValue('
                    SELECT c.id_category FROM `'._DB_PREFIX_.'category` c
                    LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.id_category = cl.id_category AND cl.id_lang = '.(int)$this->defaultLanguageId.')
                    WHERE cl.name = "'.pSQL($subGroupName).'" AND c.id_parent = '.(int)$id_group
                );
                
                if (!$id_subgroup) {
                    $subCategory = new Category();
                    $subCategory->name = array_fill_keys(Language::getIDs(true), $subGroupName);
                    $subCategory->link_rewrite = array_fill_keys(Language::getIDs(true), Tools::str2url($subGroupName));
